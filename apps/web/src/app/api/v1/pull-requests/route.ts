...
export async function GET() {
  try {
    if (useEdgeApi()) return proxyEdge("pull-requests");
    return ok({ items: await listPullRequests() });
  } catch (e) {
...
export async function POST(req: Request) {
  try {
    if (useEdgeApi()) return proxyEdge("pull-requests", { method: "POST", body: await req.text(), headers: { "Content-Type": "application/json" } });
